...
export function QuickActions() {
  return (
    <Card className="shadow-md animate-slide-up" style={{ animationDelay: "400ms" }}>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
          {actions.map((action) => (
            <Link key={action.name} to={action.href}>
              <Button
                variant="ghost"
                className={`flex h-auto w-full flex-col items-center gap-2 py-4 ${action.bg} transition-all`}
              >
